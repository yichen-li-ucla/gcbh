...
def generate_O_site(r0, distance=None, width=None, center_of_mass=None):
    """
    r0 :
        is the position of Ag atom, the equilibrium distance of O and Ag is 1.36+0.70=2.31
    center_of_mass:
    ...
    """
    if distance is None:
        distance = BOND_LENGTHS['Au'] + BOND_LENGTHS['O']
    if width is None:
        width = distance*0.15
    ...
    if center_of_mass is None:
        ...
        # new O atoms only exist in the z+ direction
    ...
    try:
        ...
        print("Whoops, Bad things happened when generate new O positions")
        exit(1)
    if center_of_mass is None:
        return dp0 + np.array(r0)  # returned value is a possible site for O atom
    else:
        ...
        if np.power(pout, 2).sum() < 1.0e-1:
            # r0 is too close to the center of mass? this is a bad Ag site
...
def add_molecule_on_cluster(cluster, molecule='O', anchor_atom=None, metal="Au",
    ...
    for n_trial in range(max_attempts_1):
        ...
        if site_symbol not in adsorption_sites:
            ...
            for n_trial_2 in range(max_attempts_2):
                ...
                if outer_sphere:
                    p1 = generate_O_site(p0, distance=BOND_LENGTHS[site_symbol]+BOND_LENGTHS[anchor_atom],
                                         width=0.05, center_of_mass=cluster.get_center_of_mass())
                else:
                    p1 = generate_O_site(p0, distance=BOND_LENGTHS[site_symbol]+BOND_LENGTHS[anchor_atom],
                                         width=0.05, center_of_mass=None)
                if p1 is None:
                    # I did not find a good O position
                    continue
                if _mole.get_number_of_atoms() == 1:
...
def remove_one_adsorbate(structures=None, molecule="O", adsorbate_weights=None):
    """
...
def add_atom_on_surfaces(surface,element="O", zmax=None, zmin= None, minimum_distance=0.7,
...
def migrate_an_adsorbate(atoms=None,molecule="O",nattempt=100,adsorption_site="Au",max_bond=1.4,min_bond=0.7):
