...
def mutation_atoms(atoms=None,dr_percent=0.6,minimum_displacement=0.5,max_trial=500,verbosity=False,elements=None):
    """
    ...
    for _i_trial in range(max_trial):
        ...
        dx_trans=[2.56*np.random.uniform(-1.0,1.0),1.91*np.random.uniform(-1.0,1.0),0.74*np.random.uniform(-1.0,1.0)] # 1.50 3.25 0.54 for rutile
        for j, sj in enumerate(symbols):
            if sj not in move_elements:
                ...
                dx[j]=rmax*np.random.uniform(-1.0,1.0,3)+dx_trans
